...
func (s SpecialList) ToBytes(_ any) []byte {
	...
	common.WriteBinary(buffer, s.Header)

	for _, pair := range s.Miis {
...
func MakeSpecialList(pool *pgxpool.Pool, ctx context.Context, contestId uint32) error {
	...
	return common.Write(bestList, fmt.Sprintf("contest/%d/special_list.ces", contestId))
}
